...
def compute_elo(G, n, K=16):
    ...
    R = pd.DataFrame({'ID':list(range(0, n)), 'Elo': compute_ratings(G, n, K)})
    #########################################
    return R
    #-----------------
    '''
...
def rank_teams_Elo(X):
    #########################################
    ## INSERT YOUR CODE HERE (5 points)
    X.insert(0, "Elo_Ranking", X['Elo'].rank(ascending = 1), True)
    Y = X.sort_values(by='Elo_Ranking', ascending=False)
